...
function opsSimulationContent(panel) {
  const preliminary = `
    <p class="ops-review-help">Consulta el resultado de la evaluación preliminar del cliente.</p>
    <section class="ops-preliminary-result">
      <div class="ops-preapproved"><small>Monto preaprobado</small><strong>S/ 250,000.00</strong><span>Cuota inicial mínima: <b>10%</b> &nbsp;&nbsp; Plazo máximo: <b>60 meses</b></span></div>
      <div class="ops-qualified"><i>✓</i><div><small>Resultado de calificación</small><strong>CALIFICA</strong><span>El cliente cumple con los criterios de evaluación.</span></div></div>
      <div class="ops-review-stat-grid"><div><small>Segmento de riesgo</small><strong>PREFERENTE</strong></div><div><small>Ingreso estimado</small><strong>S/ 5,850.00</strong></div><div><small>Capacidad de cuota máxima</small><strong>S/ 4,095.00</strong></div></div>
      <div class="ops-compliance"><div><small>PEP</small><strong>No aplica</strong><span>El cliente no está identificado como PEP.</span></div><div><small>PLAFT</small><strong>Sin observaciones</strong><span>El cliente no presenta observaciones en listas restrictivas.</span></div></div>
    </section>`;
  const calculation = `
    <p class="ops-review-help">Parámetros y resultado de crédito registrados por el Ejecutivo financiero.</p>
    <section class="ops-review-card ops-credit-parameters"><h2>Parámetros del crédito</h2>
      <div class="ops-review-subcard"><h3>Vehículo / operación</h3><div class="ops-review-grid">${opsReviewField('Tipo de cambio','3.78')}${opsReviewField('Precio del vehículo ($)','$/. 25,000.00')}${opsReviewField('Monto de cuota inicial','$/. 5,000.00')}${opsReviewField('Plazo seleccionado','36 meses')}</div></div>
      <div class="ops-review-subcard"><h3>Seguros</h3><div class="ops-review-grid">${opsReviewField('Tipo de seguro vehicular','Con Seguro')}${opsReviewField('Porcentaje anual de seguro vehicular','2.50 %')}${opsReviewField('Seguro de desgravamen','Con seguro')}${opsReviewField('Tipo de seguro desgravamen','Individual')}</div></div>
      <div class="ops-review-subcard"><h3>GPS</h3><div class="ops-review-grid">${opsReviewField('GPS','Con GPS')}${opsReviewField('Tipo de GPS','Premium')}${opsReviewField('Costo asociado ($)','$/. 650.00')}</div></div>
      <div class="ops-review-subcard"><h3>Gastos</h3><div class="ops-review-grid">${opsReviewField('Gastos registrales','Sí')}${opsReviewField('Gastos notariales','Sí')}${opsReviewField('Toma de firmas','Sí')}${opsReviewField('Portes','No')}${opsReviewField('Cuotas dobles','No')}</div></div>
      <div class="ops-review-subcard"><h3>Ingresos</h3><div class="ops-review-grid">${opsReviewField('Ingreso estimado','S/ 5,850.00')}${opsReviewField('Ingreso declarado (Soles)','S/ 5,850.00')}${opsReviewField('N° de teléfono','987654321')}${opsReviewField('Monto a financiar','S/ 20,650.00')}${opsReviewField('Día de pago','15')}</div></div>
      <section class="ops-calculation-result"><h2>Resultado del cálculo</h2><div class="ops-result-table"><b>Plazo</b><b>TEA</b><b>Cuota</b><b>CME</b><b>Capacidad</b><span>36 meses</span><span>12.80%</span><span>S/ 686.91</span><span>S/ 4,095.00</span><span class="ok">Cumple</span></div><div class="ops-result-info"><div>Carretera aplicable<strong>FULL</strong></div><div>Documentos requeridos<strong>DNI · Recibo de servicios · Cotización</strong></div><div>Verificación aplicable<strong>Verificación domiciliaria</strong></div></div></section>
    </section>`;
  return `
    <section class="ops-simulation-meta">${opsReviewField('N° de solicitud',currentCase.solicitud)}${opsReviewField('Tipo de documento','DNI')}${opsReviewField('N° de documento',currentCase.documento)}${opsReviewField('Fecha y hora de consulta',formatDate(currentCase.fecha))}</section>
    <div class="ops-simulation-tabs"><button class="${panel==='preliminary'?'active':''}" onclick="renderExecutiveOpsReview('simulation','preliminary')"><span>01</span><strong>Resultado de evaluación preliminar</strong><small>Evaluación del cliente</small></button><button class="${panel==='calculation'?'active':''}" onclick="renderExecutiveOpsReview('simulation','calculation')"><span>02</span><strong>Cálculo</strong><small>Parámetros y cuotas</small></button></div>
    ${panel === 'calculation' ? calculation : preliminary}`;
}
...
function opsSolicitudContent() {
  ...
  return `
    <section class="ops-request-summary"><b>SOLICITUD: ${escapeOpsReview(currentCase.solicitud)}</b><b>ETAPA: OPERACIONES</b><b>ESTADO: ${escapeOpsReview(String(currentCase.estado || 'Pendiente').toUpperCase())}</b><b>CARRETERA: <em>${escapeOpsReview(currentCase.carretera)}</em></b><b>FECHA Y HORA: ${escapeOpsReview(formatDate(currentCase.fecha))}</b></section>
    <section class="ops-review-card"><h2>DATOS DE CLIENTE</h2>
      <details class="ops-executive-accordion"><summary><span>Titular<br><strong>${escapeOpsReview(currentCase.cliente)}</strong></span><em>Completo</em></summary><div class="ops-accordion-body"><section><h3>Datos personales</h3><div class="ops-review-grid">${opsReviewField('Tipo de documento','DNI')}${opsReviewField('Número de documento',currentCase.documento)}${opsReviewField('Nombres',currentCase.cliente)}${opsReviewField('Género','Masculino')}${opsReviewField('Estado civil',currentCase.estadoCivil)}${opsReviewField('País de nacimiento','Perú')}</div></section><section><h3>Datos de contacto</h3><div class="ops-review-grid">${opsReviewField('Teléfono','987654321')}${opsReviewField('Correo electrónico','juan.perez@gmail.com')}</div></section><section><h3>Dirección domiciliaria · Validada</h3><div class="ops-review-grid">${opsReviewField('Departamento','Lima')}${opsReviewField('Provincia','Lima')}${opsReviewField('Distrito','San Miguel')}${opsReviewField('Dirección','Av. La Marina 2450')}</div></section><section><h3>Datos laborales</h3><div class="ops-review-grid">${opsReviewField('Categoría laboral','Dependiente')}${opsReviewField('RUC de empleador','20123456789')}${opsReviewField('Centro laboral','Comercial Andina S.A.C.')}${opsReviewField('Ingresos netos mensuales','S/ 5,850.00')}</div></section></div></details>
      <details class="ops-executive-accordion"><summary><strong>DATOS DE VEHÍCULO</strong><em>Completo</em></summary><div class="ops-accordion-body"><section><h3>Datos del vehículo</h3><div class="ops-review-grid">${opsReviewField('Estado vehículo','Nuevo')}${opsReviewField('Concesionario',currentCase.concesionario)}${opsReviewField('Sucursal',currentCase.tienda)}${opsReviewField('Tipo Doc. vendedor','DNI')}${opsReviewField('N° Doc. vendedor','45678912')}${opsReviewField('Nombre completo vendedor','Carlos Mendoza Ruiz')}${opsReviewField('Marca',brand)}${opsReviewField('Modelo',model)}${opsReviewField('Año modelo','2026')}${opsReviewField('Tarjeta propiedad a nombre de','Titular')}${opsReviewField('Color','Plata Metálico')}${opsReviewField('Tipo de vehículo','Automóvil')}${opsReviewField('VIN','BAIDAA3G512345678')}${opsReviewField('N° de motor','2ZR-458796321')}</div><div class="ops-vehicle-guarantee"><button type="button" disabled>Garantía generada</button></div></section></div></details>
      <details class="ops-executive-accordion"><summary><strong>DATOS DEL CRÉDITO</strong><em>Completo</em></summary><div class="ops-accordion-body"><section><h3>Crédito y simulación</h3><div class="ops-review-grid">${opsReviewField('Producto','Crédito Vehicular')}${opsReviewField('Campaña comercial','SUV Mayo 2026')}${opsReviewField('Tipo cambio','3.78')}${opsReviewField('Precio vehículo','$/. 25,000.00')}${opsReviewField('Cuota inicial','$/. 5,000.00')}${opsReviewField('Plazo','36 meses')}${opsReviewField('Día de pago','15')}${opsReviewField('Total financiamiento','S/ 20,650.00')}</div></section></div></details>
    </section>
...
function renderExecutiveOpsReview(stage = 'solicitud', panel = 'preliminary') {
  const container = $('executiveOpsReview');
  if (!container || !currentCase) return;
  container.innerHTML = `<div class="executive-ops-stage-nav"><button class="${stage==='simulation'?'active':''}" onclick="renderExecutiveOpsReview('simulation','preliminary')"><span>01</span> SIMULACIÓN</button><button class="${stage==='solicitud'?'active':''}" onclick="renderExecutiveOpsReview('solicitud')"><span>02</span> SOLICITUD</button></div>${stage === 'simulation' ? opsSimulationContent(panel) : opsSolicitudContent()}`;
...
$('detailSubtitle').textContent=`${currentCase.cliente} · ${currentCase.documento}`;
  ...
  if (currentCase.estado === 'Activado') {
    ...
    renderExecutiveOpsReview('solicitud');
  }
